...
export function CanvasViewport({ children }) {
    ...
    const { applySnap, clampZoom, screenToCanvas, 
    // mousePos,
    setMousePos, setShowGrid, setViewport, showGrid, viewport, } = useCanvas();
    ...
    const applyCircleResize = (current) => {
        if (!resizeState)
            return;
        const dx = current.x - resizeState.startCanvas.x;
        const dy = current.y - resizeState.startCanvas.y;
        ...
        if (handle.includes("w"))
            radius = initial.r - dx;
        if (handle.includes("s"))
            radius = Math.max(radius, initial.r + dy);
        if (handle.includes("n"))
            radius = Math.max(radius, initial.r - dy);
        patch({
    ...
    function applyEllipseResize(current) {
        if (!resizeState)
            return;
        if (element.type !== "ellipse")
            return;
        const dx = current.x - resizeState.startCanvas.x;
        const dy = current.y - resizeState.startCanvas.y;
        if (!("rx" in resizeState.initial) || !("ry" in resizeState.initial))
            return;
     ...
     * - Ellipse
     */
    const applyElementDrag = (current) => {
     ...
     * SelectionOverlay
     */
    return (_jsxs("div", { className: `relative flex-1 min-h-0 w-full bg-slate-950 overflow-hidden border border-slate-800 rounded-lg shadow-2xl ${cursorClass}`, onMouseDown: handleMouseDown, onMouseLeave: handleMouseUp, onMouseMove: handleMouseMove, onMouseUp: handleMouseUp, onWheel: handleWheel, ref: containerRef, children: [_jsx(Rulers, { containerSize: containerSize }), _jsx(CoordinateDisplay, {}), _jsxs("button", { className: "absolute bottom-4 left-10 bg-slate-900/80 text-slate-300 px-3 py-1 rounded text-xs z-30 border border-slate-700 hover:bg-slate-800", onClick: () => setShowGrid((prev) => !prev), type: "button", children: ["Grid: ", showGrid ? "ON" : "OFF"] }), _jsx("svg", { className: "w-full h-full", children: _jsxs("g", { transform: `translate(${viewport.x}, ${viewport.y}) scale(${viewport.zoom})`, children: [_jsx(Grid, { containerSize: containerSize }), _jsx(Crosshair, { containerSize: containerSize }), children, _jsx(SelectionOverlay, { element: element, onResizeStart: handleResizeStart })] }) })] }));
}
